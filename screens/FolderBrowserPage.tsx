...
import { scanMusicLibrary, MUSIC_EXTENSIONS, ScanProgress } from '../utils/MusicScanner';
          ...
          <View style={styles.folderTopRow}>
            ...
            {folder.hasNew > 0 && (
              <View style={styles.newBadge}>
                <Text style={styles.newBadgeText}>+{folder.hasNew} NEU</Text>
              </View>
            )}
            {folder.size && !folder.hasNew ? <Text style={styles.folderSize}>{folder.size}</Text> : null}
          </View>
...
export default function FolderBrowserPage() {
  ...
  const loadFolders = useCallback(async () => {
    ...
    try {
      ...
      // Load Engine DJ analysis map for BPM/key enrichment (file:// mode only)
      const engineMapPromise = isSafUri(storedPath)
        ? Promise.resolve(new Map<string, { bpm: number | null; key: string | null }>())
        : loadEngineAnalysis(storedPath);

      // Run recursive scan using MusicScanner (handles both SAF and file:// paths)
      const result = await scanMusicLibrary(storedPath, (p) => setScanProgress(p));

      const engineMap = await engineMapPromise;

      const folderList: Folder[] = result.folders.map((stat, i) => {
        // Count analysed tracks via engine map (filename-based lookup)
        const analysed = isSafUri(storedPath)
          ? 0
          : stat.tracks.filter(t => engineMap.has(t.name.toLowerCase())).length;

        return {
          id: String(i),
          name: stat.name,
          tracks: stat.trackCount,
          analysed,
          hasNew: stat.newCount,
          ...
          {scanProgress ? (
            <>
              <Text style={styles.loadingText}>
                {scanProgress.found} Tracks in {scanProgress.scanned} Ordnern gefunden…
              </Text>
              {scanProgress.currentPath ? (
                <Text style={styles.scanCurrentPath} numberOfLines={1}>
                  {scanProgress.currentPath}
                </Text>
              ) : null}
            </>
          ) : (
            <Text style={styles.loadingText}>Musikbibliothek wird gescannt…</Text>
          )}
        </View>
      ) : (
        <FlatList
